	...
	document.onreadystatechange = function(){
		...
		if(document.readyState=="complete"){
			...
			if(esReasegurable){
				...
				var combo = document.getElementById('sntroRgoAutoView.swEventoReaseg');
	
				if (swApertura == 1 || (swApertura == 0 && combo.selectedIndex == 0)) {
	
					// Si es apertura o es modificacion y no se ha informado evento
					document.getElementById('sntroRgoAutoView.swEventoReaseg').selectedIndex = 0;
					...
					document.getElementById('sntroRgoAutoView.codigoEventoRea').readOnly = 'readonly';
					document.getElementById('sntroRgoAutoView.descEventoRea').readOnly = 'readonly';
...
//					document.getElementById('sntroRgoIndusView.codigoEventoRea').disabled = true;
    ...
    function parametersEventoReaseguro() {
    	return '&codigo=' + getValue('sntroRgoAutoView.codigoEventoRea') +
          	'&descripcion=' + getValue('sntroRgoAutoView.descEventoRea');
    }
    ...
    function controlEvento(combo) {	
    	...
    	if(esReasegurable){
    		...
    		if (isEmptyField(combo) || document.getElementById('sntroRgoAutoView.swEventoReaseg').selectedIndex == 0) {
		    	...
		    	if (swExpPpal && swApertura == 0){
		    		if (!confirm(msgEliminarExpPpalEvento)){
		    			document.getElementById('sntroRgoAutoView.swEventoReaseg').selectedIndex = 1;
		    			throw "exit";
		    		}
		    	...
		    	document.getElementById('checkSwExpPrincipal').checked = '';
		    	document.getElementById('sntroRgoAutoView.idEventoRea').value = '';
		    	document.getElementById('sntroRgoAutoView.codigoEventoRea').value = '';
		    	document.getElementById('sntroRgoAutoView.descEventoRea').value = '';
    ...
    function controlEventoPrincipal(check) {
    	...
    	if(esReasegurable){
		    ...
		    if (!isEmptyField(check)) {
		    	...
		    	document.getElementById('sntroRgoAutoView.codigoEventoRea').value = '';
		    	document.getElementById('sntroRgoAutoView.descEventoRea').value = '';
		    	document.getElementById('sntroRgoAutoView.codigoEventoRea').readOnly = '';
		    	document.getElementById('sntroRgoAutoView.descEventoRea').readOnly = '';
	        ...
	        if (combo.selectedIndex == 1) {	
			    ...
			    if (obliDescEvento == 'S') {
			    	...
			    	// Check marcado. Descripcion obligatoria
			   		...
			   		document.getElementById('sntroRgoAutoView.swExpedientePrincipal').value = 'S';
				
			    } else {
			    	
			    	
			    	//Check sin marcar. Elegir evento obligatorio
			    	var idEvento = document.getElementById('sntroRgoAutoView.idEventoRea');
			    	if (isEmptyField(idEvento)) {
			    		alert(msgSeleccionaEvento);
				       	throw "exit";
			    	}
			    	document.getElementById('idEventoReaseguroExistente').value  = idEvento.value;
			    	document.getElementById('sntroRgoAutoView.swExpedientePrincipal').value = 'N';
