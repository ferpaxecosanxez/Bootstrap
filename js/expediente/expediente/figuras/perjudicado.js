...
function controlBloqueCompaniaAbogadosContrarios() {
	...
	if (document.getElementById('perjudicadoView.swAsegurado').value == valorSi) {
    	perjudicadoForm['perjudicadoView.aseguradora.id'].selectedIndex = 0;
    	perjudicadoForm['perjudicadoView.aseguradora.id'].onchange();
		perjudicadoForm['perjudicadoView.aseguradoraNombre'].value = "";
		perjudicadoForm['perjudicadoView.contrarioPoliza'].value = "";
		perjudicadoForm['perjudicadoView.contrarioRefExp'].value = "";
		perjudicadoForm['perjudicadoView.swPrepAbogado'].selectedIndex = 0;
    	perjudicadoForm['perjudicadoView.swPrepAbogado'].onchange();
		perjudicadoForm['perjudicadoView.nombreAbogado'].value = "";
		perjudicadoForm['perjudicadoView.tfnoAbogado'].value = "";
		perjudicadoForm['perjudicadoView.emailAbogado'].value = "";
		if (perjudicadoForm['perjudicadoView.tramitador.id'] != null) {
			perjudicadoForm['perjudicadoView.tramitador.id'].selectedIndex = 0;
		}
	}
}
...
function mostrarOcultarChecksYRadiosPerjudicado(mostrar){
	if(mostrar == false){
		...
		perjudicadoForm['perjudicadoView.swSiniestros'].value = "0";
	}
...
function grabar(operacion, addPerjNV){
  ...
  if (document.getElementById('figuraFormView.razonSocial') != null){		
	...
	if (operacion=='anterior'){		
		...
		var validado = validaForm(document.forms(0),validatePerjudicadoForm,'iAreaTrabajo');
		if(validado && validaTramitador()){
		    muestraCarga();
		    if (addPerjNV != undefined && addPerjNV == 'S') {
	      		document.forms(0).action += '?addPerjNV=S';
	      	    } else {
	      		document.forms(0).action += '?addPerjNV=N';	      	
	      	    }		    
