...
function guardarModificacion(){
	...
	var validado = validaForm(document.forms(0),validatePerjudicadoPersonaForm,'iAreaTrabajo');
	if(validado){
		muestraCarga();
		submitForm(document.forms(0),null,'iAreaTrabajo');
	}      
...
function enviarDatos(operacion, addPerjPer){
	if (document.getElementById('perjudicadoPersonaView.condicionLesionado.id') != null){		
		document.getElementById('perjudicadoPersonaView.condicionLesionado.id').disabled = false;
	}
	handlerBlock('figuraPerjudicadoPersona','expandir', true);
	...
	if(validado && validaTramitador()){
		muestraCarga();
      	if (addPerjPer != undefined && addPerjPer == 'S') {
      		document.forms(0).action += '?addPerjPer=S';
      	} else {
      		document.forms(0).action += '?addPerjPer=N';	      	
      	}
		submitForm(document.forms(0),null,'iAreaTrabajo');
	}        
}
...
function reloadEstadoMotivo(){   
    retrieveURLParameterSync(actionReloadEstadoMotivo,'perjudicadoPersonaForm','perjudicadoPersonaView.estadoObjeto.id','estadoObjetoAjax'); 
}
