/*
 * FUNCIONES JAVASCRIPT COMUNES DE LA OPERATIVA CAUSAS ASOCIADAS A FRANQUICIA CONSULTA
 */
    ...
    window.onload = function()	{
		if (document.getElementById('criterioSeleccion').value == "Producto"){
			...
			chequearFiltroCXF(2,'rbFamilia','rbProducto','idFamiliaProd','codigoProducto','idProducto','cIdFamiliaProd','cCodProdComponente');
		}else{
			//se activa el radio button de familia
			document.forms(0).rbFamilia.checked = "true";
			chequearFiltroCXF(1,'rbFamilia','rbProducto','idFamiliaProd','codigoProducto','idProducto','cIdFamiliaProd','cCodProdComponente');
	...
	function buscarProductoFXA(pagInicial,pagBusqueda,codProducto,idProducto){
	  var pag;
	...
	function chequearFiltroCXF(valor,rbFamilia,rbProducto,idFamilia,codProducto,idProducto,cFamilia,cProducto){
		switch(valor){
	...
	/* Funcion para la busqueda de producto: ventana emergente invocada por las lupas de producto */
	function busquedaCausas()
	{
	  var codigoSntro = document.getElementById('codigoSntro').value;
	  var descripcionSntro = document.getElementById('descripcionSntro').value;
	  var pag;
  
	  // si se ha introducido algun valor en codigo o descripcion, se deberá ejecutar directamente la consulta
	  if((codigoSntro=="")&&(descripcionSntro=="")){
		  pag = sAccionCausasInicio;
	  }else{
		  pag = sAccionCausasBuscar;
	  }
	  
	  pag= pag+"?codigoSntro="+codigoSntro+"&descripcionSntro="+descripcionSntro;
	   
	  var valor = lanzarVentana(pag,600,400);
	  if(valor != undefined) {
				
		setValue("codigoSntro", valor[0]);
		setValue("descripcionSntro",valor[1]);
		setValue("idCausaSntro",valor[2]);
	  }
	} 
