/*
 * FUNCIONES JAVASCRIPT COMUNES DE LA OPERATIVA GRUPOS NATURALEZAS X TRÁMITES CONSULTA
 */
    ...
    window.onload = function()	{
		if (document.getElementById('criterioSeleccion').value == "Producto"){
			...
			chequearFiltroGXT(2,'rbFamilia','rbProducto','idFamiliaProd','codigoProducto','idProducto','cIdFamiliaProd','cCodProdComponente');
		}else{
			//se activa el radio button de familia
			document.forms(0).rbFamilia.checked = "true";
			chequearFiltroGXT(1,'rbFamilia','rbProducto','idFamiliaProd','codigoProducto','idProducto','cIdFamiliaProd','cCodProdComponente');
	...
	function buscarProductoGXT(pagInicial,pagBusqueda,codProducto,idProducto){
	  var pag;
	...
	function chequearFiltroGXT(valor,rbFamilia,rbProducto,idFamilia,codProducto,idProducto,cFamilia,cProducto){
		switch(valor){
